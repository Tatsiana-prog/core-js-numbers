...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const numbers1 = x1 * x2 + y1 * y2;
  const numbers2 = Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2);
  const angle = Math.acos(numbers1 / numbers2);
  return angle;
}
/**
...
function getFloatOnString(str) {
  const floatNumber = Number.parseFloat(str);
  return Number.isNaN(floatNumber) ? NaN : floatNumber;
}
/**
...
function getIntegerOnString(str, base) {
  const integerNumber = Number.parseInt(str, base);
  return Number.isNaN(integerNumber) ? NaN : integerNumber;
}
/**
